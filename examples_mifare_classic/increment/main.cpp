/**
 * @file
 * @brief     Example use of the nfc library to increment a value block on a Mifare classic 1k using the pn532
 * 
 * This file will demonstrate how to use the nfc library to increment valueblock with the pn532
 * 
 ...
 * a succesfull authentication, it will continue to increment the value stored in the valueblock by a specified value
 * 
 * The pn532 will first send out the increment command to the mifare classic. After a succesfull incrementation the page is not yet changed. The 
 * incrementation result is only temporarily stored in a specific buffer on the Mifare card. In order to transfer the data form the internal buffer
...
int main() {
    ...
    uint32_t incrementValue         = 50;                                                   // Value the valueblock needs to be incremented by
    ...
    for(;;){
        ...
        auto increment = nfc->mifareIncrement(cardinfo, cardnumber, athenticateAorB, valueBlockPage, sector, sectorKey, incrementValue);
        if(increment !=   nfc::statusCode::pn532StatusOK){hwlib::cout << "Error incrementing" << hwlib::endl;continue;}
